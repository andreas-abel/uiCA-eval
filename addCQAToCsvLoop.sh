...
run_CQA() {
   ...
   tp=`../maqao.intel64.2.16.0/maqao.intel64 cqa $codeFile fct-loops=.* uarch=$uarch --confidence-levels=expert | grep 'each iteration of the binary loop takes' | head -n 1 | sed -E 's/.*takes (.*) cycles.*/\1/' | tr -d "."`
   rm $codeFile $asmFile
   echo "$line,$tp"
}
export -f run_CQA

...
def getAssemblerCode(filename):
   output = subprocess.check_output(['objdump', '-b', 'binary', '-Mintel,x86-64', '-m', 'i386', '--no-show-raw-insn', '-D', filename])
   lines = output.split('\n')[7:-1]
...
def main():
   ...
   parser.add_argument('-unroll', help="unroll", type=int, default=1)
   ...
   for line in lines[1:]:
      ...
      output = subprocess.check_output([xedBinary, '-64', '-v', '4', '-ir', 'code'])
